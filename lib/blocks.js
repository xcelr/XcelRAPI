...
BlockController.prototype.getBlockByHash = function(hash, next) {
    ...
    if (blockCached) {
        ...
        blockCached.confirmations = self.node.services.xcelrd.height - blockCached.height + 1;
        ...
        return async.waterfall([function (callback) {
            ...
            return self.node.services.xcelrd.getBlockHeader(hash, function (err, info) {
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix xclcore to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.xcelrd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
      ...
      return async.setImmediate(function() {
        ...
        return async.waterfall([function (callback) {
            return self.node.services.xcelrd.client.getBlock(hash, function (err, response) {
      ...
      if (err) {
        ...
        var confirmations = self.node.services.xcelrd.height - block.height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  var moreTimestamp = lte;
    ...
    return self.node.services.xcelrd.getBlockHashesByTimestamp(lte, gte, function (err, hashes) {
